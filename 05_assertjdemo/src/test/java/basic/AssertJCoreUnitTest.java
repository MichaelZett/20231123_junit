...
class AssertJCoreUnitTest {
    ...
    void whenComparingFields_thenEqual() throws Exception {
        Dog fido = new Dog("Fido", 5.15f);
        Dog fidosClone = new Dog("Fido", 5.15f);
        Dog lightFido = new Dog("Fido", 3.3F);

        assertThat(fido).usingRecursiveComparison().ignoringFields("weight").isEqualTo(lightFido);
    ...
    void whenRunningAssertion_thenDescribed() throws Exception {
        ...
        assertThat(person.getAge()).as("% age should be equal to 100", person.getName()).isEqualTo(100);
    }
}
